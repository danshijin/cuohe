...
import com.smm.cuohe.dao.ISupplyDao;
...
@Service
public class SupplyBO implements ISupplyBO {

	@Resource
	private ISupplyDao iSupplyDao;
	@Resource
	...
	public List<Supply> getAll(Map<String, String> map) {
		...
		List<Supply> list = this.iSupplyDao.getAll(map);
		for (Supply c : list) {
			if (StringUtils.isNotBlank(c.getEntTypes())) {
				...
				for (Map<String, Object> aListMap : listMap) {
					enType = enType + aListMap.get("name") + ",";
				}
	...
	public Integer getAllCount(Map<String, String> map) {
		...
		Integer i = -1;
		try {
			i = this.iSupplyDao.getAllCount(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return i;
	...
	public int deleteByIds(String ids) {
		return this.iSupplyDao.deleteByIds(ids);
	}
	...
	public String importExcel(List<Map<String, String>> dataInfo, User loginUser) {
		...
		for (Map<String, String> item : dataInfo) {
			try {
				...
				// 企业员工人数
				String companyCategoryEmployeeStr = item
						.containsKey("companyCategoryEmployee") ? item.get(
						"companyCategoryEmployee").trim() : "";
				Integer categoryEmployeeInt = 0;
				if (StringUtils.isBlank(companyCategoryEmployeeStr)) {
					errorTemp += "企业员工人数不为空；";
				} else {
				...
				// 客户级别 客户状态
				String customStatusStr = item.containsKey("customStatus") ? item
						.get("customStatus").trim() : "";
				Integer customLevelInt = 0;
				if (StringUtils.isBlank(customStatusStr)) {
					errorTemp += "客户状态不为空；";
				} else {
					List<Category> listCustomLevel = new ArrayList<Category>();
					if (mapCategory.containsKey("客户级别"))
						listCustomLevel = mapCategory.get("客户级别");
					for (Category customLevelItem : listCustomLevel) {
						if (customLevelItem.getName().equals(customStatusStr)) {
							customLevelInt = customLevelItem.getId();
							break;
						}
					}
					if (customLevelInt == 0) {
				...
				customInfo.setLevel(customLevelInt);
	...
	public List<Supply> getSupplyExportData(String[] array) {
		List<Supply> list = this.iSupplyDao.getSupplyExportData(array);
		for (Supply c : list) {
			if (StringUtils.isNotBlank(c.getEntTypes())) {
				List<Map<String, Object>> listMap = pDao.getCategList(c
						.getEntTypes());
